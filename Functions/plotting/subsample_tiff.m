function [tiff, X, Y] = subsample_tiff(tiff, tiff_info, gridsize)
    ...
    if dims < 3
        % Create interpolant
        xgrid = linspace(tiff_info.XWorldLimits(1), tiff_info.XWorldLimits(2), size(tiff, 2));
        ygrid = linspace(tiff_info.YWorldLimits(1), tiff_info.YWorldLimits(2), size(tiff, 1));
        [X, Y] = meshgrid(xgrid,ygrid);
        F = griddedInterpolant(X', Y', double(tiff(:, :))', 'nearest', 'none');
