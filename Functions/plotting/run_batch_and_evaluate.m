function [] = run_batch_and_evaluate(axes, md_list)
    ...
    md_names = {'SMB 3 yr avg.', 'Iteration 0', 'Iteration 2', 'Iteration 4'};
    ...
    plotmodel(md0, 'data', mdb.results.TransientSolution(end).Thickness - md0.geometry.thickness, ...
                'data', mds.results.TransientSolution(end).Thickness - md0.geometry.thickness, ...
                'data', mdw.results.TransientSolution(end).Thickness - md0.geometry.thickness, ...
    ...
    vel_b = cell2mat({mdb.results.TransientSolution(end).Vel});
    vel_s = cell2mat({mds.results.TransientSolution(end).Vel});
    vel_w = cell2mat({mdw.results.TransientSolution(end).Vel});
    ...
    plotmodel(md0, 'data', vel_b - md0.inversion.vel_obs, ...
                'data', vel_s - md0.inversion.vel_obs, ...
                'data', vel_w - md0.inversion.vel_obs, ...
                'caxis#all', [-2e2 2e2], 'mask#all', mdb.results.TransientSolution(end).MaskIceLevelset<0, ...
                'xticklabel#all', ' ', 'yticklabel#all', ' ', ...
