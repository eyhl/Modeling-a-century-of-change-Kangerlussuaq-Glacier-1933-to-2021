function [interp_data] = figure2_plot(md, fig_nr)
    save_figures = false;
    ...
    years = 1985:2018;
    [transient_errors, ice_masks, model_vel, obs_vel] = get_transient_vel_errors(vel_model, vel_data, t_model, years, [], ice_mask, 'yearly', md);
    ...
    for i=1:length(years)
        ...
        model_velocity(:, i) = interpToFlowline(md.mesh.x, md.mesh.y, model_vel(:, i), x_flowline, y_flowline);
        obs_velocity(:, i) = interpToFlowline(md.mesh.x, md.mesh.y, obs_vel(:, i), x_flowline, y_flowline);
        % interp_data(:, i) = model_velocity(:, i) - obs_velocity(:, i);
        % 
